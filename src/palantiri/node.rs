...
impl Node {
    ...
    async fn handle_reorg(&self, new_block: BlockHeader) -> Result<(), NodeError> {
        let state = self.SyncedState
            .as_ref()
            .ok_or(NodeError::State("Not in synced state".into()))?
            .read()
            .await;
    
        let current_block = self.rpc.get_block_by_number(state.current_block, false)
            .await
        ...
        if new_block.number <= state.current_block {
            ...
            while old_chain.parent_hash != new_chain.parent_hash {
                old_chain = self.rpc.get_block_by_hash(old_chain.parent_hash, false)
                    .await
                    .map_err(|e| NodeError::Rpc(e.to_string()))?
                    .header;
    
                new_chain = self.rpc.get_block_by_hash(new_chain.parent_hash, false)
                    .await
                    .map_err(|e| NodeError::Rpc(e.to_string()))?
                    .header;
            }
            ...
            // Get common ancestor
            let ancestor = self.rpc.get_block_by_hash(old_chain.parent_hash, false)
                .await
                .map_err(|e| NodeError::Rpc(e.to_string()))?
                .header;
    
            // Emit reorg event
            self.event_tx.send(ChainEvent::Reorg {
    ...
    async fn initialize(&self) -> Result<(), NodeError> {
        let genesis = self.rpc.get_block_by_number(0, false)
            .await
            .map_err(|e| NodeError::Rpc(e.to_string()))?;

        let mut state = self.SyncedState.as_ref().ok_or(NodeError::State("SyncedState not initialized".to_string()))?.write().await;
    ...
    pub async fn sync_blocks(&mut self) -> Result<(), NodeError> {
        let mut state = self.NotSyncedState.as_ref().ok_or(NodeError::State("SyncedState not initialized".to_string()))?.write().await;
        self.SyncedState = None;
       
        let latest = self.rpc.get_block_number()
            .await
            .map_err(|e| NodeError::Rpc(e.to_string()))?.as_u64().unwrap();
    ...
    async fn sync_block_range(&self, mut start: u64, end: u64) -> Result<(), NodeError> {
        while start <= end {
            ...
            for number in start..=end {
                let block = self.rpc.get_block_by_number(number, false)
                    .await
                    .map_err(|e| NodeError::Rpc(e.to_string()))?;

                // Verify block header
                self.consensus.verify_block(block.header.parent_hash)
                    .await
    ...
    async fn watch_new_blocks(&self) -> Result<(), NodeError> {
        ...
        loop {
            ...
            let block = self.rpc.get_block_by_number(latest.as_u64().unwrap(), true)
                .await
                .map_err(|e| NodeError::Rpc(e.to_string()))?;
